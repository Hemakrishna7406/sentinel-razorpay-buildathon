...
def write_ablation_report(results, modes):
    with open("evaluation/experiments/behavioral_evasion_results.md", "w") as f:
        ...
        for mode, name in modes.items():
            if mode == "full_sentinel":
                continue  # Skip governance in detection layer
            m = results[mode]["overall"]
            pr_auc = results[mode]["aucpr"]
            f.write(
                f"| {name} | {m['precision']:.3f} | {m['recall']:.3f} | {m['f1']:.3f} | {pr_auc:.3f} | {m['far']:.3%} |\n"
            )
        ...
        f.write(
            f"| **Full Sentinel** | {m['precision']:.3f} | {m['recall']:.3f} | {m['f1']:.3f} | {pr_auc:.3f} | {m['far']:.3%} |\n"
        )
