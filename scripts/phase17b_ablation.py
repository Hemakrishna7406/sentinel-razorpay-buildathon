...
def run_phase17_ablation():
    print("Loading frozen dataset v2...", flush=True)
    df = pd.read_parquet("evaluation/dataset/sentinel_v2.parquet")
...
def calc_subset_metrics(y_true, y_pred, y_prob, mask):
    if not np.any(mask):
        return {"precision": 0, "recall": 0, "f1": 0, "far": 0, "aucpr": 0, "has_positive": False}
    ...
    has_positive = len(np.unique(y_t)) > 1 and np.max(y_t) == 1
    aucpr = average_precision_score(y_t, y_prob_sub) if has_positive else 0
...
def format_pr_auc(val, has_positive):
    if not has_positive:
        return "N/A"
    return f"{val:.3f}"

def write_ablation_report(results, modes):
    with open("evaluation/experiments/behavioral_evasion_results.md", "w") as f:
        f.write("# Phase 17b Behavioral-Evasion Evasion Study\n\n")
        f.write("## Overall Performance (Detection Layer)\n")
        f.write("| System | Precision | Recall | F1 | PR-AUC | False Escalation |\n")
        f.write("|--------|-----------|--------|----|--------|------------------|\n")
        
        for mode, name in modes.items():
            if mode == "full_sentinel": continue # Skip governance in detection layer
            m = results[mode]["overall"]
            pr_auc = results[mode]["aucpr"]
            f.write(f"| {name} | {m['precision']:.3f} | {m['recall']:.3f} | {m['f1']:.3f} | {pr_auc:.3f} | {m['far']:.3%} |\n")
            
        f.write("\n## Governance Layer (Full Sentinel with Policy)\n")
        f.write("| System | Precision | Recall | F1 | PR-AUC | False Escalation |\n")
        f.write("|--------|-----------|--------|----|--------|------------------|\n")
        
        m = results["full_sentinel"]["overall"]
        pr_auc = results["full_sentinel"]["aucpr"]
        f.write(f"| **Full Sentinel** | {m['precision']:.3f} | {m['recall']:.3f} | {m['f1']:.3f} | {pr_auc:.3f} | {m['far']:.3%} |\n")
        ...
        for mode, name in modes.items():
            if mode == "full_sentinel": continue
            seen = results[mode]["seen"]
            unseen = results[mode]["unseen"]
            seen_val = format_pr_auc(seen["aucpr"], seen["has_positive"])
            unseen_val = format_pr_auc(unseen["aucpr"], unseen["has_positive"])
            f.write(f"| {name} | {seen_val} | {unseen_val} |\n")
