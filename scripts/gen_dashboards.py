...
security = {
    ...
    "panels": [
        ...
        },
        {
            ...
            "options": {"colorMode": "background"},
            "fieldConfig": {"defaults": {"thresholds": {"mode": "absolute", "steps": [
                {"color": "green", "value": 0}, {"color": "yellow", "value": 1}
            ]}}}
        },
        {
        ...
        },
        {
            ...
            "targets": [{"expr": 'rate(sentinel_decisions_total{decision="ESCALATE"}[5m])', "legendFormat": "{{reason}}"}]
        },
    ]
}
...
executive = {
    ...
    "panels": [
        ...
        {"id": 6, "type": "timeseries", "title": "Risk Score Percentiles",
...
performance = {
    ...
    "panels": [
        ...
        {"id": 6, "type": "timeseries", "title": "Component Latency Breakdown (worker p99)",
         "gridPos": {"h": 8, "w": 24, "x": 0, "y": 12},
         "targets": [
             {"expr": "histogram_quantile(0.99, rate(sentinel_worker_queue_wait_seconds_bucket[5m])) * 1000", "legendFormat": "Queue Wait p99"},
             {"expr": "sentinel:ml_inference_p99:5m * 1000", "legendFormat": "XGBoost p99"},
             {"expr": "histogram_quantile(0.99, rate(sentinel_redis_latency_seconds_bucket[5m])) * 1000", "legendFormat": "Redis p99"},
             {"expr": "histogram_quantile(0.99, rate(sentinel_postgres_latency_seconds_bucket[5m])) * 1000", "legendFormat": "Postgres p99"},
         ]},
    ]
}
...
worker = {
    ...
    "time": {"from": "now-1h", "to": "now"},
    "panels": [
        {"id": 1, "type": "gauge", "title": "Worker Utilization",
         "gridPos": {"h": 6, "w": 6, "x": 0, "y": 0},
        ...
        {"id": 2, "type": "stat", "title": "Active Tasks",
         "gridPos": {"h": 6, "w": 6, "x": 6, "y": 0},
         "targets": [{"expr": "sentinel_worker_active_tasks", "legendFormat": "Active"}]},
        {"id": 3, "type": "stat", "title": "Capacity (MAX_CONCURRENT_TASKS)",
         "gridPos": {"h": 6, "w": 6, "x": 12, "y": 0},
         "targets": [{"expr": "sentinel_worker_capacity", "legendFormat": "Capacity"}]},
        {"id": 4, "type": "timeseries", "title": "Kafka Consumer Lag",
         "gridPos": {"h": 8, "w": 24, "x": 0, "y": 6},
         "targets": [{"expr": "sentinel_kafka_lag", "legendFormat": "{{topic}}[{{partition}}]"}]},
        {"id": 5, "type": "timeseries", "title": "Queue Wait p99 (ms)",
         "gridPos": {"h": 8, "w": 12, "x": 0, "y": 14},
         "targets": [{"expr": "histogram_quantile(0.99, rate(sentinel_worker_queue_wait_seconds_bucket[5m])) * 1000", "legendFormat": "Queue Wait p99"}]},
        {"id": 6, "type": "timeseries", "title": "Dependency Errors/s",
