...
def print_header():
    ...
    console.print(Panel.fit(
        "[bold cyan]Sentinel[/bold cyan] - Fail-Closed Authorization & Containment Layer",
        border_style="cyan"
    ))
    console.print()
...
def run_deterministic_scenario(name: str, desc: str, decision: str, reason: str, token: str, mcp: str, status: str = None):
    ...
    if token:
        console.print(f"  [bold]Token:[/bold]    {'[bold green]ISSUED[/bold green]' if token == 'ISSUED' else '[bold red]NOT ISSUED[/bold red]'}")
    if mcp:
        console.print(f"  [bold]MCP Exec:[/bold] {'[green]SUCCESS[/green]' if 'SUCCESS' in mcp else '[red]' + mcp + '[/red]'}")
    if status:
...
if __name__ == "__main__":
    ...
    run_deterministic_scenario(
        "Benign Transaction", "Agent performing typical, in-pattern activity.",
        "ALLOW", "Cleared all policy and fusion checks.", "ISSUED", "SUCCESS"
    )
    run_deterministic_scenario(
        "Suspicious Transaction (Drift)", "Agent exhibits velocity and amount drift, but semantics are not inherently malicious.",
        "ESCALATE", "High behavioral drift (Velocity x31)", "NOT ISSUED", "BLOCKED (No token)"
    )
    run_deterministic_scenario(
        "Malicious Transaction (Containment)", "Agent exhibiting severe behavioral anomalies AND semantic risk.",
        "CONTAIN", "Severe behavioral anomalies AND semantic risk (Unseen rapid exfiltration)", "NOT ISSUED", "BLOCKED (No token)"
    )
    run_deterministic_scenario(
        "Idempotency / Replay Attack", "Attacker resends the exact same transaction payload (Benign).",
        "ESCALATE", "A request with this idempotency key is already published.", "NOT ISSUED", "BLOCKED (No token)", "FAILED REPLAY"
    )
    run_deterministic_scenario(
        "UNKNOWN Timeout (MCP Failure)", "Gateway times out during execution (Execution_Unknown). No automatic retries.",
        "ALLOW", "Cleared all policy and fusion checks.", "ISSUED", "FAILED", "NO AUTOMATIC RETRY (Reconciliation Required)"
    )
