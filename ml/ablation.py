...
def _generate_ablation_markdown(results: Dict[str, Any]):
    ...
    with open("experiments/ablation.md", "w", encoding="utf-8") as f:
        ...
        f.write("> **Research Question (H1):** Does incorporating agent-level behavioral context improve detection of autonomous financial abuse over transaction-level signals alone?\n\n")
        ...
        if full_auc > tx_auc * 1.05:
            f.write("✅ **H1 Accepted:** Behavioral context significantly improves detection performance compared to transaction-only signals.\n")
        elif full_auc >= tx_auc:
            f.write("⚠️ **H1 Weakly Accepted:** Behavioral context provides marginal improvement over transaction-only signals.\n")
        else:
            f.write("❌ **H0 Accepted:** Behavioral context does not improve detection performance in this synthetic environment.\n")
            
        f.write("\n## Model Selection\n\n")
        params = results['full_sentinel']['params']
        f.write(f"The `full_sentinel` model was selected as the final artifact.\n")
        f.write(f"- Selected Hyperparameters: max_depth={params.get('max_depth', 'N/A')}, lr={params.get('learning_rate', 'N/A')}\n")
