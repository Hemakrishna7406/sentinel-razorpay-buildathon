...
async def get_kpis(db: Session = Depends(get_db)):
    ...
    records = (
        ...
        .filter(AuditRecord.timestamp >= last_24h)
        .first()
    )
...
async def get_fraud_events(
    days: int = Query(7, ge=1, le=90), limit: int = Query(100, ge=1, le=1000), db: Session = Depends(get_db)
):
    """
    ...
    results = (
        ...
        .filter(AuditRecord.timestamp >= start_time, AuditRecord.decision != "ALLOW")
...
async def get_financial_summary(days: int = Query(30, ge=1, le=90), db: Session = Depends(get_db)):
    """
    ...
    total = (
        db.query(func.count(AuditRecord.id), func.sum(AuditRecord.amount))
        .filter(AuditRecord.timestamp >= start_time)
        .first()
    )

    prevented = (
        db.query(func.sum(AuditRecord.amount))
        .filter(AuditRecord.timestamp >= start_time, AuditRecord.decision != "ALLOW")
        .first()
    )
