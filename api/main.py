...
from api.schema import PolicyRuleRequest, PolicyRuleResponse, SimulateRequest, SimulateResponse
...
async def security_exception_handler(request: Request, exc: SentinelSecurityException):
    ...
    # Always fail closed
    return JSONResponse(status_code=403, content={"decision": "ESCALATE", "reason": str(exc), "executed_tx_id": None})
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={"decision": "ESCALATE", "reason": "Unhandled system error. Failing closed.", "executed_tx_id": None},
    )
    ...
    with tracer.start_as_current_span("sentinel.evaluate") as root_span:
        ...
        with tracer.start_as_current_span("sentinel.kafka.publish"):
            ...
            try:
                await kafka_producer.send_and_wait(
                    KAFKA_INBOUND_TOPIC, key=intent.agent_id.encode("utf-8"), value=json.dumps(payload).encode("utf-8")
                )
        ...
        with tracer.start_as_current_span("sentinel.redis.poll"):
            try:
                ...
                try:
                    REDIS_LATENCY.labels(operation="xread_reply").observe(time.perf_counter() - t_redis_start)
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                logger.error("Error during synchronous evaluation", intent_id=intent.intent_id, error=str(e))
                raise
...
async def evaluate_intent_async(
    ...
    if existing_tx:
        return JSONResponse(
            status_code=200, content={"intent_id": intent.intent_id, "status": "COMPLETED", "tx_id": existing_tx}
        )
...
async def run_demo_scenario(scenario: str, count: int = 10, api_key: str = Depends(optional_api_key)):
...
def add_policy_rule(
    req: PolicyRuleRequest, policy_engine=Depends(get_policy_engine), admin_key: str = Depends(require_admin_key)
):
...
def delete_policy_rule(
    rule_id: str, policy_engine=Depends(get_policy_engine), admin_key: str = Depends(require_admin_key)
):
...
def run_simulation(
    ...
    if len(test_df) == 0:
        return SimulateResponse(
            total=0, allowed=0, escalated=0, contained=0, avg_risk_score=0.0, escalation_rate=0.0, decisions=[]
        )
    ...
    for _, row in test_df.iterrows():
        ...
        fusion = RiskFusionEngine(base_escalation_threshold=getattr(model_wrapper, "suspicious_threshold", 0.5)).fuse(
            behavioral, None
        )
...
def serve_landing():
    ...
    return HTMLResponse(
        content="<h1>Frontend build not found. Please run 'npm run build' in frontend/</h1>", status_code=404
    )
...
def get_audit_log(limit: int = 50, db: Session = Depends(get_db), admin_key: str = Depends(require_admin_key)):
...
def verify_audit_log(db: Session = Depends(get_db), admin_key: str = Depends(require_admin_key)):
...
def get_security_invariants(db: Session = Depends(get_db), api_key: str = Depends(optional_api_key)):
    """
...
async def health_ready():
    ...
    try:
        ...
        logger.warning("DB unavailable during readiness check — audit consumer will reconcile", error=str(e))
