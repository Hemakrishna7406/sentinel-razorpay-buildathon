...
async def test_19l_reconciliation_endpoint():
    ...
    receipt = ExecutionReceipt(
        execution_id="fail_123", intent_id="intent_123", decision_id="ALLOW",
        capability_jti="jti_123", agent_id="a", requested_action="refund",
        mcp_tool="update_refund", requested_amount=100, executed_amount=0,
        currency="INR", provider="razorpay-mcp", environment="test",
        status="UNKNOWN", latency_ms=0, verification_status="VERIFIED",
        timestamp="now", provider_reference="Timeout"
    )
...
async def test_19m_orphaned_lock_reclamation(mock_redis):
    ...
    mock_redis.get.return_value = json.dumps({
        "intent_hash": engine._hash_intent(valid_intent),
        "state": "FAILED",
        "tx_id": None
    })
