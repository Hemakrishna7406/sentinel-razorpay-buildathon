...
async def test_redis_unavailable_fails_closed():
    intent = IntentRequest(
        intent_id="int_fail_1", agent_id="agent_1", action_type="payout",
        amount=100, currency="INR", recipient="rec_1"
    )
...
def test_capability_token_tampering():
    ...
    req_valid = ExecuteRequest(
        intent_id="int1", agent_id="ag1", action_type="payout",
        amount=1000, currency="INR", recipient="rec1",
        capability_token=token, idempotency_key="ik1"
    )
    def to_ctx(req):
    ...
    req_tamper_amount = ExecuteRequest(
        intent_id="int1", agent_id="ag1", action_type="payout",
        amount=10000, currency="INR", recipient="rec1",
        capability_token=token, idempotency_key="ik2"
    )
    ...
    req_tamper_currency = ExecuteRequest(
        intent_id="int1", agent_id="ag1", action_type="payout",
        amount=1000, currency="USD", recipient="rec1",
        capability_token=token, idempotency_key="ik3"
    )
    ...
    req_tamper_intent = ExecuteRequest(
        intent_id="int2", agent_id="ag1", action_type="payout",
        amount=1000, currency="INR", recipient="rec1",
        capability_token=token, idempotency_key="ik4"
    )
