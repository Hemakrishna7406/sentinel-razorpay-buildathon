    ...
    async with AsyncClient(transport=ASGITransport(app=api.main.app), base_url="http://test") as ac:
        ...
        for r in responses:
            if r.status_code == 200:
                data = r.json()
                if data.get("executed_tx_id"):
                    executions.add(data["executed_tx_id"])
                    
        assert len(executions) == 1, f"Expected exactly 1 execution, got {len(executions)} unique executions"
...
async def test_fail_closed_redis_outage():
    ...
    api.main.get_idempotency_engine = lambda: IdempotencyEngine(redis_client=OutageMockRedis(), behavioral_window_seconds=5)
